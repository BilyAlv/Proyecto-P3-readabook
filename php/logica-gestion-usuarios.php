...
function handleCriticalError($message, $errorDetails = null) {
    ...
    // En desarrollo muestra detalles, en producción solo mensaje genérico
    $displayMessage = $_SERVER['SERVER_NAME'] == 'localhost' ? 
        "Error: $message - " . ($errorDetails ?? '') : 
        "Ha ocurrido un error. Contacte al administrador.";
    
    // Almacena mensaje en sesión para mostrarlo después de redirección
    $_SESSION['error_message'] = $displayMessage;
    
    // Decide si redirigir o mostrar mensaje
    if (!headers_sent()) {
...
if (!isset($_SESSION['id']) || $_SESSION['rol'] !== 'admin') {
    // Guarda mensaje de error y URL original para redirigir después del login
...
// Verificar tiempo de inactividad (15 minutos)
